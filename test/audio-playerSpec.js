...
describe('Audio player', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(AudioPlayer.prototype, 'playbackCompatibility');
    spyOn(AudioPlayer.prototype, 'getPlayer');
  });
  ...
  it('should construct with event functionality', () => {
    ...
    expect(player.playbackCompatibility).toHaveBeenCalledTimes(1);
    expect(player.getPlayer).toHaveBeenCalledTimes(1);
    const callbacks = player.getPlayer.calls.mostRecent().args[0];
  ...
  describe('Compatibility', () => {
    ...
    it('should recognize playback compatibility when all are available', () => {
      ...
      const player = new AudioPlayer();
      player.playbackCompatibility.and.callThrough();
      player.playbackCompatibility();
  ...
  it('should change the playbackrate of the audio', () => {
    const player = new AudioPlayer();
    player.player = jasmine.createSpyObj('player', ['setPlaybackRate']);
    player.setPlaybackRate(2);
    expect(player.player.setPlaybackRate).toHaveBeenCalledTimes(1);
    expect(player.player.setPlaybackRate).toHaveBeenCalledWith(2);
  });

  it('should get the playbackrate of the audio', () => {
    const player = new AudioPlayer();
    player.player = jasmine.createSpyObj('player', ['getPlaybackRate']);
    player.getPlaybackRate();
    expect(player.player.getPlaybackRate).toHaveBeenCalledTimes(1);
  });

  it('should set the volume of the audio', () => {
    const player = new AudioPlayer();
    player.player = jasmine.createSpyObj('player', ['setAudioVolume', 'getAudioVolume']);
    player.setAudioVolume(0.2);
    expect(player.player.setAudioVolume).toHaveBeenCalledTimes(1);
    expect(player.player.setAudioVolume).toHaveBeenCalledWith(0.2);
    expect(player.audioMuted).toBeFalsy();
  });
  ...
  it('should set the volume of the audio to zero and mute', () => {
    ...
    expect(player.player.setAudioVolume).toHaveBeenCalledTimes(1);
    expect(player.player.setAudioVolume).toHaveBeenCalledWith(0);
    expect(player.audioMuted).toBeTruthy();
  });

  it('should get the volume of the audio', () => {
    const player = new AudioPlayer();
    player.player = jasmine.createSpyObj('player', ['setAudioVolume', 'getAudioVolume']);
    player.player.getAudioVolume.and.returnValue(0.5);
    const result = player.getAudioVolume(0.2);
    expect(player.player.getAudioVolume).toHaveBeenCalledTimes(1);
  ...
  it('should unmute the audio after muting', () => {
    ...
    player.player.getAudioVolume.and.returnValue(0);
    player.setAudioMute(false);
    expect(player.audioMuted).toBeFalsy();

    expect(player.player.setAudioVolume).toHaveBeenCalledTimes(2);
    expect(player.player.setAudioVolume).toHaveBeenCalledWith(0);
    expect(player.player.setAudioVolume).toHaveBeenCalledWith(0.5);
  });
  ...
  it('should unmute the audio with a previous changed audiolevel', () => {
    ...
    const player = new AudioPlayer();
    player.player = jasmine.createSpyObj('player', ['setAudioVolume', 'getAudioVolume']);
    player.player.getAudioVolume.and.returnValue(audioLevel);
    player.setAudioVolume(audioLevel);
    player.setAudioMute(true);
    player.setAudioMute(false);
    expect(player.player.setAudioVolume).toHaveBeenCalledWith(0);
    expect(player.player.setAudioVolume).toHaveBeenCalledWith(audioLevel);
    expect(player.player.setAudioVolume).toHaveBeenCalledTimes(3);
  });

  it('should get the audioMuted property', () => {
    const player = new AudioPlayer();
    const result = player.isAudioMuted();
    expect(result).toBeFalsy();
  });

  it('should get the audioMuted property after muting', () => {
    const player = new AudioPlayer();
    player.player = jasmine.createSpyObj('player', ['setAudioVolume', 'getAudioVolume']);
    player.player.getAudioVolume.and.returnValue(0.5);
    player.setAudioMute(true);
  ...
  it('should toggle the mute state on and off', () => {
    const player = new AudioPlayer();
    player.player = jasmine.createSpyObj('player', ['setAudioVolume', 'getAudioVolume']);
    player.player.getAudioVolume.and.returnValue(1);
    ...
    player.toggleAudioMute();
    expect(player.player.setAudioVolume).toHaveBeenCalledTimes(2);
    expect(player.player.setAudioVolume).toHaveBeenCalledWith(1);
  });
});
