...
describe('Pronunciation Analysis Challenge Recording API', () => {
      ...
      analysis
        ...
        .then(() => {
          ...
          expect(getRequest.args).toEqual([
            'GET',
            '/challenges/pronunciation/ch4/analyses/r3c',
          ]);
          done();
        })
        .catch(done.fail);
  ...
  describe('prepareChallenge', () => {
    it('should call pronunciation.init_challenge with analysisId and challengeId', done => {
      ...
      analysis
        .prepareChallenge('analysisId', 'challengeId')
        .then(() => {
          expect(makeWebsocketCallSpy).toHaveBeenCalledWith(
            'pronunciation.init_challenge',
            {
  ...
  describe('prepareAudio', () => {
    ...
    it('should call pronunciation.init_audio with audio specs', done => {
      analysis
        .prepareAudio('analysisId', recorderStub)
        .then(() => {
          expect(makeWebsocketCallSpy).toHaveBeenCalledWith(
            'pronunciation.init_audio',
            {
  ...
  describe('streamAudio', () => {
    it('should call to encodeAndSendAudioOnDataAvailable with analyseId and a recorder', done => {
      const encodeAndSendAudioOnDataAvailableSpy = spyOn(
        utils,
      ...
      analysis
        .streamAudio('analyseId', 'noRecorder')
        .then(() => {
          expect(encodeAndSendAudioOnDataAvailableSpy).toHaveBeenCalledWith(
            'analyseId',
            'noRecorder',
            'pronunciation.write',
          );
  ...
  describe('endStreamAudio', () => {
    it('should call pronunciation.analyse with analysisId and progressCb', done => {
      ...
      analysis
        .endStreamAudio('analyseId', 'progressCb')
        .then(() => {
          expect(makeWebsocketCallSpy).toHaveBeenCalledWith(
