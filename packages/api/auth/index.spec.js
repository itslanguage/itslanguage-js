...
describe('assembleScope', () => {
  it('should always require arguments', () => {
    expect(() => auth.assembleScope()).toThrowError(
      Error,
      'Arguments are required to assemble scope.',
    );
  });

  it('should build a valid scope for a top level admin', () => {
    expect(auth.assembleScope(null, null, 'superadmin')).toEqual(
      'user/superadmin',
    );
  });

  it('should build a valid scope for a tenant only', () => {
    expect(auth.assembleScope('rotterdam')).toEqual('tenant/rotterdam');
  });

  it('should build a valid scope for a tenant user only', () => {
    expect(auth.assembleScope('rotterdam', null, 'lee')).toEqual(
      'tenant/rotterdam/user/lee',
    );
  });

  it('should build a valid scope for an organisation in a tenant', () => {
    expect(auth.assembleScope('rotterdam', 'towers')).toEqual(
      'tenant/rotterdam/organisation/towers',
    );
  });

  it('should build a valid scope for an user in an organisation in a tenant', () => {
    expect(auth.assembleScope('rotterdam', 'towers', 'lee')).toEqual(
      'tenant/rotterdam/organisation/towers/user/lee',
    );
  });
});
...
describe('impersonate', () => {
  ...
  it('should make a post request and update the settings', done => {
    ...
    authorisedRequestSpy.and.returnValue(
    ...
    auth
      .impersonate('this/is/not/a/valid/scope')
      .then(() => {
...
describe('authenticate', () => {
  ...
  it('should make a post request and update the settings', done => {
    ...
    auth
      .authenticate('foo', 'bar', 'this/is/not/a/valid/scope')
      .then(() => {
